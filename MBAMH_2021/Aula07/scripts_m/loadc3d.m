...
% STICKFIGURE_C3D - Retrieves 3D coordinate/analog data from a C3D file 
%
% Input: 
% FullFileName       File name of trial
%
% Outputs:
...
% Ver. 2.0 Revision (Jaap Harlaar, Amsterdam, April 2002) [renamed to "readc3d.m"]
% Ver. 2.1 Revision (Jasper Menger, Enschede, February 2005) [adapted for files with Float32-data, speed improved, renamed to "loadc3d"]
...
wait_txt      = [FileName, ' is loading...'];
wait_txt      = strrep(wait_txt, '_', ' '); % replace underscores to prevent subscript
%Wbar          = waitbar2(0, wait_txt);
NvideoFrames  = EndFrame - StartFrame + 1;
Markers       = zeros(NvideoFrames, Nmarkers, 3);
AnalogSignals = zeros((NanalogFramesPerVideoFrame * NvideoFrames), NanalogChannels);
...
if Scale < 0    
    ...
    for i = 1:NvideoFrames
        % Marker and AD data
        DataM = fread(fid, [4, Nmarkers], 'float32')';
        DataA = fread(fid, [NanalogChannels, NanalogFramesPerVideoFrame], 'float32')';
        Markers(i, 1:Nmarkers, 1:3) = DataM(1:Nmarkers, 1:3);
        i_start = NanalogFramesPerVideoFrame * (i - 1) + 1;
        i_stop  = NanalogFramesPerVideoFrame * i;
        AnalogSignals(i_start:i_stop, 1:NanalogChannels) = DataA;
        % Camera Info and Residual Error
        Byte(1:Nmarkers) = fix(squeeze(DataM(1:Nmarkers, 4)));  
        HighByte         = fix(Byte / 256);
        LowByte          = Byte - HighByte * 256; 
        CameraInfo(i, 1:Nmarkers)    = HighByte(1:Nmarkers); 
        ResidualError(i, 1:Nmarkers) = LowByte(1:Nmarkers) * abs(Scale); 
%        waitbar2(i / NvideoFrames);
    end    
else
    ...
    for i = 1:NvideoFrames
        ...
        DataA   = fread(fid, [NanalogFramesPerVideoFrame, NanalogChannels], 'int16');
        i_start = NanalogFramesPerVideoFrame * (i - 1) + 1;
        i_stop  = NanalogFramesPerVideoFrame * i;
        AnalogSignals(i_start:i_stop, 1:NanalogChannels) = DataA;
%       waitbar2(i / NvideoFrames);
